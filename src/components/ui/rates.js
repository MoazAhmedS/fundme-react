...
function StarRating({ stars = 5, initialRating = 0, starColor = 'text-yellow-500' }) {
    ...
    <div className="flex">
      {[...Array(stars)].map((_, index) => {
        ...
        return (
          <svg
            key={index}
            className={`w-6 h-6 cursor-pointer ${currentRating <= (hover || rating) ? starColor : 'text-gray-300'}`}
            fill="currentColor"
            viewBox="0 0 20 20"
